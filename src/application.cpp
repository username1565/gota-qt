...
const char VERSION_DATA_URL[] = "https://raw.githubusercontent.com/noritte/gota-gui/master/LatestStableVersion.txt?1"; // use ?1 trick to force reload and bypass cache
...
{
    setApplicationName("gota"); // do not change because it also changes data directory under Mac and Win
    setApplicationDisplayName(tr("Gota Wallet") + ' ' + Settings::getFullVersion());
...
bool WalletApplication::init()
{
    ...
    QString path = dataDir.absoluteFilePath("gota-gui.lock");
    m_lockFile.reset(new QLockFile(path));

    if (!m_lockFile->tryLock())
    {
        WalletLogger::warning(tr("[Application] Gota wallet GUI already running"));
        QMessageBox::warning(nullptr, QObject::tr("Error"), tr("Gota wallet GUI already running"));
        return false;
    }
...
{
    splashMsg(tr("Looks like this is your first run of the new Gota Wallet GUI. It is in detached state now.\nYou can open a wallet file, create a new one, or connect to remote walletd daemon."));
}
