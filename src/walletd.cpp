...
void BuiltinWalletd::run()
{
    ...
    if (Settings::instance().getGotadConnectionMethod() != ConnectionMethod::BUILTIN)
        args << QString{"--gotad-remote-address=%1"}.arg(Settings::instance().getGotadEndPoint());
...
QString BuiltinWalletd::errorMessage(ReturnCodes err)
{
    ...
    switch(err)
    {
    case ReturnCodes::GOTAD_DATABASE_ERROR:
        msg = tr("Database write error. Disk is full or database is corrupted.");
        break;
    case ReturnCodes::GOTAD_ALREADY_RUNNING:
        msg = tr("Cannot run gotad. Another instance of gotad is running.");
        break;
    case ReturnCodes::WALLETD_BIND_PORT_IN_USE:
        msg = tr("Cannot run walletd. Walletd bind port in use.");
        break;
    case ReturnCodes::GOTAD_BIND_PORT_IN_USE:
        msg = tr("Cannot run gotad. Gotad bind port in use.");
        break;
    case ReturnCodes::GOTAD_WRONG_ARGS:
        msg = tr("Wrong arguments passed to gotad.");
        break;
    ...
    case ReturnCodes::WALLET_FILE_EXISTS:
        msg = tr("The specified wallet file already exists. Gota wallet never overwrites an existed wallet file for safety reason. If you want to overwrite the file please remove it manually and try again\n(") + Settings::instance().getWalletFile() + ").";
        break;
