...
constexpr char OPTION_GOTAD_CONNECTION_METHOD[] = "gotadConnectionMethod";
constexpr char OPTION_GOTAD_HOST[] = "gotadHost";
constexpr char OPTION_GOTAD_PORT[] = "gotadPort";
...
constexpr quint16 DEFAULT_MAIN_WALLETD_RPC_PORT = 38082;
...
constexpr quint16 DEFAULT_MAIN_GOTAD_RPC_PORT = 38081;
constexpr quint16 DEFAULT_TEST_GOTAD_RPC_PORT = DEFAULT_MAIN_GOTAD_RPC_PORT + 1000;
constexpr quint16 DEFAULT_STAGE_GOTAD_RPC_PORT = DEFAULT_MAIN_GOTAD_RPC_PORT + 2000;

constexpr char LOCAL_HOST[] = "127.0.0.1";

#if defined(Q_OS_LINUX)
constexpr char DEFAULT_WORK_DIR[] = ".gota";
#endif

const constexpr char* DEFAULT_MINING_POOLS[] = { "pool.gotanetwork.org:5150" };
...
Settings::Settings()
{
    ...
    const QString jsonFile = getDefaultWorkDir().absoluteFilePath("gota-gui.config");
...
ConnectionMethod Settings::getGotadConnectionMethod() const
{
//    return static_cast<ConnectionMethod>(settings_->value(OPTION_GOTAD_CONNECTION_METHOD, static_cast<int>(getGotadDefaultConnectionMethod())).toInt());
    return getEnumValue<ConnectionMethod>(OPTION_GOTAD_CONNECTION_METHOD, getGotadDefaultConnectionMethod());
}

QString Settings::getGotadEndPoint() const
{
    const quint16 port = getGotadPort();
    return QString{"%1:%2"}.arg(getGotadHost()).arg(port ? port : getDefaultGotadPort());
}

QString Settings::getGotadHost() const
{
    return settings_->value(OPTION_GOTAD_HOST).toString();
}

quint16 Settings::getGotadPort() const
{
    return static_cast<quint16>(settings_->value(OPTION_GOTAD_PORT, getDefaultGotadPort()).toUInt());
}
...
quint16 Settings::getDefaultGotadPort() const
{
    switch(getNetworkType())
    {
    case NetworkType::MAIN:  return DEFAULT_MAIN_GOTAD_RPC_PORT;
    case NetworkType::STAGE: return DEFAULT_STAGE_GOTAD_RPC_PORT;
    case NetworkType::TEST:  return DEFAULT_TEST_GOTAD_RPC_PORT;
    }
    return DEFAULT_MAIN_GOTAD_RPC_PORT;
}
...
void Settings::setGotadEndPoint(const QString& host, quint16 port)
{
    settings_->setValue(OPTION_GOTAD_HOST, host);
    settings_->setValue(OPTION_GOTAD_PORT, port);
//    settings_->setValue(OPTION_GOTAD_END_POINT, QString("%1:%2").arg(host).arg(port));
}
...
void Settings::setGotadConnectionMethod(ConnectionMethod method)
{
    settings_->setValue(OPTION_GOTAD_CONNECTION_METHOD, static_cast<int>(method));
}
...
/*static*/
ConnectionMethod Settings::getGotadDefaultConnectionMethod()
{
    return ConnectionMethod::BUILTIN;
}
